...
**`init`만으로는 `/harness-*` 슬래시 커맨드가 설치되지 않습니다.** `init`은 `commands/*.md` 25개를 프로젝트에 복사하지 않으며, 해당 명령은 Claude Code 플러그인 설치 채널에서 제공됩니다.
...
결정된 member는 `docs/<member>/` 한 세그먼트여야 한다 — 비어 있거나, `/`·`\`를 포함하거나, `.`으로 시작하거나,
제어문자가 있으면 `task`는 아무것도 쓰지 않고 거부하고 `init`은 저장하지 않는다. 한글·공백은 허용한다.
손으로 고친 `.harness/active.json`도 같은 규칙으로 읽는다 — user·task가 규칙을 어기면 활성 task 없음으로 보고 경고한다.
