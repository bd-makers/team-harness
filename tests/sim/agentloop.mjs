...
// 순수 채점 함수는 rule 층(rules.mjs)에 있다 — 이 파일에는 하네스(샌드박스·spawn·리포트 I/O)만 남는다.
import {
  sig, manual, na, ICO, renderSignals, ambiguityCounts, forceAllChecked,
  scoreSpecArtifacts, aggregateTrials,
} from './rules.mjs';
...
// (sectionBody·ambiguityCounts·forceAllChecked·scoreSpecArtifacts·aggregateTrials는
//  rule 층 rules.mjs로 이동 — 위 import 참조.)
...
// ── report ────────────────────────────────────────────────────────────────────
// (ICO·renderSignals는 rule 층 rules.mjs로 이동 — 위 import 참조.)
