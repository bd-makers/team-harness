...
2. ```bash
   harness-team task new fix <name>
   ```
3. `spec.md`에 증상·재현 경로·의심 원인 기록
4. Grep/Glob으로 관련 코드 탐색, 근본 원인 파악

### Phase 2: 수정 + 회귀 테스트
1. `plan.md`에 수정 접근 + 체크리스트 작성
...
2. 재현 시나리오로 수동 확인
3. 중요한 수정은 `CLAUDE.md`의 **코드 리뷰 기준** 체크리스트 확인 후 결과 보고

### Phase 4: 마무리
1. `/handoff` — handoff.md 갱신
2. `harness-team task done` — artifact.md에 git diff/log/test 결과 자동 수집

## 핵심 원칙
- **근본 원인 수정**, 증상만 가리는 패치 금지
- 테스트로 회귀 방지 확실히
- 관련 없는 코드는 건드리지 말 것 (별도 이슈/task로 분리)
