...
- **`docs/harness-workflow-simulation.html`의 전체 스킬 로스터에서 빠져 있던 커맨드 2종** — 위 두 개가 표에 없어
  "무엇이 있는지" 확인하러 온 독자가 그 존재를 알 수 없었다. 행을 추가하고 산문 개수를 맞췄으며, 절 제목의
  `(0.21.0)` 시점 표기는 지웠다 — 번호가 한 곳에만 있어야 틀릴 곳도 한 곳이다.
  (이 파일의 hero 배지·🆕 배너는 아래 항목에서 함께 현행화했다.)
- **`docs/harness-workflow-simulation.html`이 0.23.0에 머물러 있던 것** — hero 배지를 `v0.39.1`로 올리고
  🆕 배너에 `0.24.0–0.39.1` 4개 그룹을 추가했다. 배너가 새로 말하는 사실과 본문이 어긋나지 않도록 세 곳을
  함께 고쳤다: `done`의 리뷰·검증 증거는 `meta.reviews[]`가 정본이고 구 task만 마커로 판정한다(0.37.0),
  `--force`는 무시된 issue가 있으면 `forcedAt`·`forcedIssues`를 남긴다(0.34.0), 원장 `--write`는 브랜치 이름이
...
- **`doctor`의 결정 로그(D-log) 절 검사가 fenced code block 안의 `## D<n>` 줄을 절로 세지 않습니다** —
  ...
  (2026-09-05 codex 리뷰 P2에서 "저확률"로 미뤄 둔 후속). 검사 전에 펜스 블록을 걷어내며 판정은
  CommonMark §4.5를 따릅니다 — 닫히지 않은 펜스는 문서 끝까지 코드, 백틱 펜스의 info string에
  백틱이 있는 줄은 펜스가 아니며(그 줄을 펜스로 보면 뒤 문서를 삼켜 **거짓 누락 경고**가 납니다),
  줄 끝은 LF·CRLF·단독 CR 모두 인식합니다. 펜스 뒤의 진짜 헤딩은 그대로 절로 셉니다.

## [0.34.0] - 2026-09-09

### Added
- **`done --force` 종결에 감사 흔적이 남습니다** — 가드를 무시하고 닫은 task가 종결 이후
  정상 종결과 **기계적으로 구분되지 않던** 문제를 고칩니다. 무시한 issue는 stdout에 한 번
  출력되고 휘발해서 `<name>-meta.json`·원장·`doctor`·SessionStart 재개 후보 어디에도 남지
  않았습니다. 가설이 아닙니다 — 선행 task가 이 경로를 실제로 밟았고 우회 이력을 `plan.md`에
  **손으로** 적어 유지하고 있었습니다. meta에 `forcedAt`(ISO8601)·`forcedIssues`(무시한 issue
  원문)를 더하고, 원장은 우회 종결을 `✅ done ⚠️`로, 사용자 인덱스는 `- ✅ <task> ⚠️`로 그립니다.
  **기록 조건은 `--force` 플래그가 아니라 실제로 무시된 issue가 1개 이상인 것입니다** —
  플래그만 붙고 무시한 것이 없으면 우회가 아니므로 남기지 않습니다.

### Changed
- **원장 역파싱 정규식을 렌더링과 같은 상수에서 조립합니다** — `⚠️`는 U+26A0 + U+FE0F 두
  코드포인트라 렌더와 파서 양쪽에 손으로 적으면 한쪽에서 변이 선택자가 빠져도 **에러 없이**
  그 행만 매치에 실패합니다. 원장은 meta가 사라진 구 task의 `created`·`done` 마지막 출처라
  그 손실이 영구적입니다. 정상 종결의 표기(`✅ done`)는 접두로 보존되므로 기존 원장의 판정도
  그대로 성립합니다.
- **meta가 유실된 우회 task는 원장에서 "우회가 있었다(시각 불명)"만 복구합니다** — 시각과
  issue 원문은 지어내지 않습니다. 이 복구값은 reopen 시 meta에 굳습니다: 열린 행(`🔄 open`)은
  표시를 담을 자리가 없어, 굳히지 않으면 열린 구간에 원장을 한 번만 재생성해도 유일한 출처가
  사라지고 이후 정상 종결이 우회 없음으로 잘못 표시됩니다.
- 필드가 없는 기존 task는 **"우회 아님"이 아니라 "알 수 없음"** 으로 degrade합니다.
  렌더링은 종전과 바이트 동일하므로 원장 재생성으로 diff가 생기지 않습니다.

### 바뀌지 않는 것
- **가드의 차단 판정(`collectDoneIssues`)과 `--force` 우회 경로는 그대로입니다.** 이 릴리스는
  막지 않고 **기록만** 합니다 — 그래서 "가드의 위협 모델은 망각이지 고의가 아니다"라는
  `done-guard-window`의 확립된 결정과 충돌하지 않습니다.
- 소비자가 할 일은 없습니다. 새 필드는 다음 우회 종결 때 생깁니다.

## [0.33.0] - 2026-09-08

### Added
- **템플릿 refresh가 스킬·규칙까지 도달합니다** — 적용 범위가 정적 템플릿 16개 중 **6개에서 13개**로
  늘었습니다(`.claude/skills` 3 + `.claude/rules` 4 추가). `copyStaticAssets`가 `skipExisting`으로
  복사하는 탓에 **수정된 템플릿은 기존 설치에 영영 도달하지 않았습니다**(신규 파일은 도달 —
  `copyTree` 판정이 파일 단위라 생기는 비대칭). 그래서 `new-feature` Phase 3(수직 슬라이스 규율,
  0.32.3에 들어간 내용)이 기존 프로젝트에 배달되지 않은 채였습니다. 정책은 새로 만들지 않았습니다 —
  `refreshClaudeHooks`가 이미 쓰던 provenance 규칙(배포한 적 있는 sha256만 갱신, 나머지는
  커스터마이즈로 보아 보존)을 그대로 확장했습니다. 갱신 경로는 `harness-team migrate`입니다.
- **`doctor`가 낡은 스킬·규칙 설치본을 경고합니다** (`stale skill/rule templates`) — `next_actions`에
  `harness-team migrate`를 실어 보냅니다. 갱신 경로가 있어도 발견성이 없으면 아무도 부르지 않습니다;
  이 드리프트가 오래 눈에 띄지 않은 이유가 그것입니다. 사용자가 편집한 파일에는 경고하지 않습니다 —
  `migrate`가 고치지 않을 것을 시키면 거짓 안내이기 때문입니다.
- **결정 로그 D8** — 템플릿 갱신 정책(provenance refresh, 전면 덮어쓰기 금지)과 기각한 대안 3종,
  그리고 `legacyStock` 예외를 기록했습니다.

### Changed
- **`docs/decisions.md` 절 누락 경고가 마이그레이션 방법을 알려줍니다.** `## D8`이 검사 목록에
  들어가므로 **D8 이전에 스캐폴드된 소비자는 이 경고를 새로 받습니다.** 이 경고는 `init`·`migrate`
  어느 쪽으로도 해결되지 않습니다 — 결정 로그는 설치 후 팀이 저작하는 파일이라 하네스가 덮어쓰지
  않기 때문입니다(D8: `docs/` seed는 refresh 비목표). 문구가 그 사실과 이유, 그리고 복사해 올 원본의
  **절대 경로**를 안내합니다. 경로는 백틱으로 감쌉니다 — 플러그인이 공백 있는 경로에 설치되면
  구분자 없이는 경로 끝을 알 수 없습니다.
- **규칙 유래 검사가 stock 규칙을 건너뜁니다.** 마커 도입 이전 판은 "사용자가 스탬프를 빠뜨린 것"이
  아니라 "낡은 것"이고 처방이 다릅니다(`migrate`). 억제 전에는 같은 파일에 상충하는 지시 두 개가
  나갔습니다. 유래 없는 **사용자** 규칙은 그대로 보고합니다.

### Fixed
- **규칙 refresh가 `.cursor/rules/*.mdc` 미러를 갱신하지 않던 문제.** Claude만 새 규칙을 보고 Cursor는
  옛 규칙을 계속 읽는데, `doctor`는 `.claude`만 보므로 정상으로 돌아와 **드리프트가 스스로를
  숨겼습니다.** 외부 리뷰(Codex MAJOR)가 잡았습니다.
- **판정과 쓰기 사이의 TOCTOU, 그리고 leaf symlink 타깃 덮어쓰기.** 확인 프롬프트 사이에 파일이
  바뀌거나 leaf가 symlink로 교체되면 "stock만 덮는다"는 계약이 깨집니다. 쓰기 직전 재검증 +
  leaf symlink 거부로 막습니다. **디렉터리 symlink(공식 backup 구조)는 그대로 통과합니다.**
  외부 리뷰(Codex MAJOR)가 잡았습니다.
- **CI가 이력 의존 테스트를 검사하지 못하던 문제.** `actions/checkout` 기본값이 얕은 클론이라
  `git log`가 커밋 하나만 봤습니다. `fetch-depth: 0`으로 실제 이력을 주고, 얕은 클론에서는
  실패가 아니라 skip 하도록 고쳤습니다.
...
- **task handoff의 완료 마커가 단일 선언에서 나옵니다.** `runDone`이 쓰고 `inferLegacyMeta`가
  ...
  손으로 복사한 리터럴로만 묶여 있었고 그 계약을 지키는 테스트가 없었습니다. 실측 — 생산자의
  em-dash를 하이픈으로 바꾸면 `meta.json`이 없는 레거시 task가 `done` 대신 `open`으로 분류되는데
  609개 테스트가 전부 통과했습니다. 새 모듈 `src/handoff-marker.mjs`가 `renderDoneMarker`·
  `hasDoneMarker`를 소유하고 두 지점이 그것을 씁니다. **append 바이트와 정규식은 그대로라
  동작·파일 형식 변경은 없습니다.**
- **드리프트를 막는 것은 모듈이 아니라 두 방향의 테스트입니다.** 왕복(`hasDoneMarker(renderDoneMarker(ts))`)과
  바이트 모양이 *생산자* 드리프트를, 렌더러에서 생성하지 않은 **동결 fixture**가 *소비자* 드리프트를
  잡습니다. 후자는 파서를 좁혀 이미 디스크에 쓰인 과거 형태(`## 2026-01-01 — 완료`처럼 날짜만 있는
  마커)를 못 읽게 만드는 변경을 차단합니다 — 파서의 느슨함은 버그가 아니라 하위 호환 계약입니다.
  동시에 커밋 항목(`## <시각> — <sha 메시지>`)이 "…완료"로 끝나도 종결로 읽히지 않는다는 경계도
  테스트로 고정했습니다.
...
- **observe 루프백 nudge** — 트립와이어가 발화하면 텍스트 출력의 `next:` 줄과 `--json`의 `next_actions`
  ...
  날짜 — 창 안의 같은 사건은 재실행해도 같은 이름). 발화를 task로 이어가는 간선이 없었고
  (exit 1과 세션 로그 추적 안내로 끝남), AI-native SDLC 플레이북의 "관측 이탈 → intent 작성" 루프에
  대응합니다. **task를 자동 생성하지는 않습니다** — 임계값(20·5·2×·3회)이 실사용 보정 전이라 오탐 task를
  만들지 않기 위해서입니다.

### Changed
- **spec 인터뷰에 Problem 축** — `/harness-spec` 인터뷰 차원에 Problem(오늘 무엇이 안 되는가·누가
  영향받는가)을 추가하고, `/harness-interview` 선행 채점의 Goal pass 근거로 기대 결과 문장과 **문제 문장**을
  함께 요구합니다. 별도 체크박스·가중치는 없습니다 — Goal 차원 안에서 발사되므로 기존 task의 자가진단
  구조는 그대로입니다. spec 템플릿 목적 절에 안내문 1줄(문제 → 영향 → 기대 결과 → 제약)이 붙습니다.
- **열린 질문 규약 `(open)`** — 답을 못 받은 질문은 spec `## 참고` 절에 `- (open) …`으로 남기고, Ambiguity
  게이트 통과 전에 답이 반영되거나 `(open → <대상>)`으로 이월 대상이 적혀야 합니다. 소스 충돌의
  `(unresolved)`와 다릅니다 — 그쪽은 요구가 둘이고 선택이 남은 것, `(open)`은 답 자체가 없는 것입니다.

## [0.30.0] - 2026-09-06

### Changed
- **`summary --write` 가드가 브랜치 이름 대신 커밋을 본다** — 브랜치 이름이 기본 브랜치가
  아니어도 HEAD 커밋이 **`origin/HEAD`가 가리키는 기본 브랜치와 정확히 같으면** 원장을 씁니다.
  그런 브랜치에는 로컬 커밋이 하나도 없어 원장 커밋이 그 브랜치 바로 위에 얹히고, 기본
  브랜치에서 쓰는 것과 결과가 같습니다. git worktree 세션은 기본 브랜치를 체크아웃할 수 없어(원본 체크아웃이
  ...
  역시 거부합니다. 판정은 **`origin/HEAD`만** 근거로 삼습니다 — 이 저장소의 기본 브랜치를
  이름 대는 유일한 ref이고, 없으면 기본 브랜치를 특정할 수 없으므로 새 경로가 아예 열리지
  않습니다(이름 판정이 `main`/`master`로 폴백하는 것과 달리, 커밋 판정에서 그 폴백을 쓰면 실제
  기본이 `main`인 저장소에서 낡은 `origin/master` 위의 브랜치까지 열립니다 — behind를 거부하는
  이유와 같은 상황입니다). `origin/HEAD` 조회·`rev-parse` 어느 쪽이든 실패하면(원격 ref 없음·
  커밋이 하나도 없는 저장소·git 오류) 허용이 아니라 **기존 거부로 떨어집니다**(fail-closed).
  origin이 없는 로컬 전용 저장소의 동작은 이름 판정 그대로여서 바뀌지 않습니다. 거부 메시지도 그대로입니다 — 새 경로가 열린 뒤 남는
