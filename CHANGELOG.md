...
- **spec/plan 단계 다이어그램 옵트인.** 신규 task를 만든 직후 다이어그램을 함께 만들지 **1회만**
  ...
  **전용 설정 키(`.harness/config.json`)·doctor 체크·상태 파일을 만들지 않았다**: "예"의 상태는
  산출물 존재로, "아니오"의 상태는 plan.md에 그 단계가 없다는 사실로 이미 표현되며 plan.md는 이미
  SSOT이자 세션 시작 프로토콜이 반드시 읽는 파일이다 — **plan.md가 곧 상태다.**
