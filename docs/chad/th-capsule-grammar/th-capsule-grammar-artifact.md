...
- capsule 종결자를 CommonMark의 0~3칸 들여쓰기 `#`/`##` ATX 절 제목으로 한정하고, 같은 범위의 `####` 이하 제목은 capsule 범위에 유지했다. 4칸 이상 들여쓴 heading 유사 줄은 코드 본문으로 센다.
...
- 후속 R4 수정 전 `validateContextCard` 재현: 3칸 들여쓴 `####`는 `0`으로 유지됐지만, 4칸 들여쓴 `#### raw failure marker`도 `0`으로 과소 계수됐다.
- 후속 수정 후 `node --test tests/context.test.mjs`: 19/19 통과. 0~3칸 ATX 제목 비내용 처리, 4칸 들여쓴 코드 본문 계수, 3칸 들여쓴 `##` 절 종결을 회귀로 고정했다.
...
- 결정/조치: R4는 ATX 제목 판정 전에 들여쓰기를 제거해 코드 블록을 제목으로 오인하던 독립 카운터 결함이었다. 0~3칸 들여쓰기만 ATX 제목으로 인정하고 4칸 이상은 코드 본문으로 세며, 스캔 구조는 바꾸지 않았다.

## Learnings

- 이번 문제는 같은 ATX 줄을 종결자와 실질 내용으로 서로 다르게 해석한 두 판정의 정합성 결함이다. 셸 주석은 그 종결자 결함의 fenced-code 변형이며, R4는 ATX 제목과 들여쓴 코드 블록의 경계를 명시한 독립 카운터 결함이다. 두 경우 모두 줄 단위 스캔 구조를 유지했다.
