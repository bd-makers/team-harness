...
## 단계
- [ ] 실패 재현 — 가드 수준 회귀 그물(R2): 소스 + 자기 `<name>-spec.md`만 커밋한 task가
      **차단되어야 한다**는 테스트를 먼저 쓰고, 수정 전에 실패하는 것을 확인한다
      (fixture는 반드시 `firstActivatedAt`을 갖는다 — 없으면 공허하게 통과한다)
- [ ] 실패 재현 — 순수 함수: `docs/chad/demo/demo-spec.md` · `docs/superpowers/specs/foo.md`가
      `{ source: false, test: false }` 여야 한다는 테스트 추가, 실패 확인
- [ ] 구현 — `isTestPath()` 맨 앞에 `SOURCE_EXTENSIONS` 확장자 게이트 추가,
      `SOURCE_EXTENSIONS` 주석을 "판정 양쪽을 지배한다"로 갱신
- [ ] 회귀 확인 — 기존 판정 8건(R3) assertion 무수정 통과 + 전체 스위트 0 fail
- [ ] 순서 계약 고정 — 게이트가 디렉터리 규칙보다 앞에 있음을 테스트로 못 박음(구멍 2 재발 방지)
