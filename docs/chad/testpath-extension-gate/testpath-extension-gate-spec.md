...
- `tests/` **안**의 산문 판정은 목록 기반이라 원리적으로 열려 있다 — 목록에 없는 문서 포맷은
  테스트로 세어진다. 2차 리뷰(P2)에서 지적된 실제 회피 경로 3종은 닫았다:
  dotfile(`tests/.gitignore` → 도구 설정), 끝의 점(`tests/README.md.` → 확장자 정규화),
  목록 부족(`org`·`tex`·`typ`·`asciidoc`·`textile` 추가). 남는 것은 **알려지지 않은 새 포맷**뿐이고,
  실패 방향이 **차단이 아니라 통과**라 `--force` 상습화를 유발하지 않는다(아래 비대칭 참조).
