...
- **`tests/perf/boundary-checkpoint.test.mjs`는 부하에 민감한 flake다 — 내용 무관 확증.**
  CI에서 4회 관측했다: main `32801750964`(test 20), PR #41 `32802292903`(test 20),
  PR #41 `32802551963`(**test 18** — 같은 브랜치에서 실패 job이 뒤집혔다).
  #41은 diff가 task 문서(md/json)뿐이라 테스트를 깨는 것이 원리적으로 불가능하고,
  실패 job이 Node 18↔20 사이를 오간다 — **Node 버전 특이성이 아니라 러너 자원 경합**이다
  (matrix 두 job이 동시에 뜬다). 매번 실패 job만 재실행하면 통과한다.
