...
### 후속 후보 — summary 가드가 워크트리에서 만족 불가 (이 task 범위 밖, 고치지 않음)

`harness-team summary --write`가 **리터럴 브랜치명 `main`**을 요구한다. 그런데 git은 같은 브랜치를
두 워크트리에 체크아웃할 수 없고, 기본 체크아웃이 `main`을 점유하고 있으면 워크트리 세션은
그 조건을 **원천적으로 만족시킬 수 없다**. 2026-08-25 집계 갱신 때 실제로 걸렸다.

가드의 의도는 "공유 원장을 feature 브랜치에서 갱신해 병렬 브랜치끼리 충돌하는 것"을 막는 것이고,
그 의도는 브랜치 **이름**이 아니라 **`HEAD`가 `origin/<기본브랜치>`와 동일한가**로도 충족된다.
이번에는 그 조건(HEAD sha == origin/main · diff 0줄 · 미커밋 0건 · 열린 PR 0건)을 입증한 뒤
`--force`로 진행했고 오케스트레이터가 결과를 대조해 승인했다 — 절차로 메운 것이지 가드가
맞았던 것은 아니다.

> `/harness-spec` 발견 (1)(2)와는 **별개 건**이다. 저 둘은 커맨드 계약 문제고 이건 CLI 가드 문제다.
