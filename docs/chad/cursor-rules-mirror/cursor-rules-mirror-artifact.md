...
| # | 심각도 | 발견 | 판별 | 조치 |
|---|---|---|---|---|
| 1 | P2 | `globs:` 값을 인용하지 않아, 첫 glob이 `**/*.ts`처럼 YAML indicator로 시작하면 생성된 frontmatter가 파싱 불가 | **결함으로 판단 — 단 전제 1개 미검증.** js-yaml 실측: `globs: **/*.ts` → `unidentified alias`, `globs: [id].tsx, ...` → `bad indentation of a mapping entry`. **이는 strict YAML 파서 기준이며, Cursor가 실제로 strict YAML로 frontmatter를 읽는지는 확인하지 못했다.** 만약 Cursor가 관대한 라인 기반 파서라면 인용부호가 리터럴로 들어가 오히려 매칭이 깨진다 | `yamlScalar()` 추가 — indicator로 시작할 때만 작은따옴표로 감싼다(일반 케이스는 Cursor 문서 형태 그대로 유지). 회귀 테스트 1건 추가. **영향 범위는 사용자 규칙 중 첫 glob이 indicator로 시작하는 경우뿐** — 동봉 템플릿 4개는 전부 `src/`·`app/` 선행이라 출력이 이전과 바이트 단위로 동일하다. 실제 Cursor 설치에서 인용된 `globs:`가 auto-attach 되는지 확인 필요 |
