...
변환 결과 실제 실행은 **66건**이다 — 변환 65건 + playground에 **추가 1건**.
추가분은 `PRESETS` 개수 가드다. 원본은 `for (const name of Object.keys(PRESETS))`로 단언을
생성하므로 `PRESETS`가 비면 단언이 0개 등록되고 **조용히 통과**한다. 이 공백을 닫았다.
기준선을 흐리지 않도록 "변환 65 + 추가 1"로 분리해 센다.
