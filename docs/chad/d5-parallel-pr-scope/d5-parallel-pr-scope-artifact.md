...
- **2026-08-20 — advisor 리뷰 (완료 직전).** 지적 4건 중 3건 조치, 1건 의도적 미조치.
  - *CI 미확인* → `gh pr checks 24` 확인. `test (18)`·`test (20)` 모두 pass.
    `.github/workflows/test.yml`은 `npm test`만 돌리고 `docs:check`는 CI에 없다.
  - *`D4`만 grep하고 `병렬`은 grep하지 않음* → 전수 확인. `README.md:68` 설계 스코프 절이 D4를
    인용하지 않은 채 같은 규칙을 재진술하고 있어 수정. `templates/docs/README.md:51`은 브랜치 병렬을
    전제한 설명이라 D5와 이미 정합적(무수정), `commands/harness-inttest.md:149`는 무관,
    `GEMINI.md` 계열은 해당 서술 없음.
  - *§2 불릿 위치가 바로 아래 "최종 판단·쓰기는 메인 세션이 담당"과 충돌하게 읽힘* → "세션 간
    얘기이며 한 세션 안에서는 위 원칙이 그대로 적용된다"를 덧붙여 구분.
  - *D5에 회귀 가드 없음* → **미조치.** 이번 task는 문서 전용 범위라 테스트를 추가하지 않고
    PR #24 코멘트에 갭으로 남겼다. 후속 판단은 리뷰어 몫.
- **2026-08-20 — 외부 리뷰(Codex/Gemini) 미실행.** 문서 텍스트 전용 변경(실행 경로 없음)이라 `harness-codex-review`를
...
- **문서 규칙을 고칠 때는 규칙을 *인용하는* 문서가 아니라 규칙을 *재진술하는* 문서를 찾아야 한다.**
  `D4`로 grep하면 D4를 명시적으로 가리키는 파일만 나온다. 정작 위험한 건 출처를 안 밝힌 채 같은
  규칙을 자기 말로 쓴 문서다(`README.md`가 그랬다). **개념 낱말(`병렬`)로 다시 grep** 해야 전수가 된다.
  쌍 드리프트 테스트는 여기서 아무 도움이 안 된다 — 루트와 템플릿이 똑같이 낡으면 그냥 통과한다.
