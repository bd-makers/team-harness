...
|---|---|---|---|---|
| 1 | P2 | 24 KiB 상수의 근거가 넓어진 측정 집합에 맞춰 재도출되지 않음 (`doctor.mjs`, `MAINTAINING.md`) | **확인.** 문장 자체는 정확("이 레포의 **프로젝트** 계층 ~16 KB")하나, 예산은 이제 그 **상위집합**에 걸린다 — 부분집합 실측으로 상위집합 예산을 정당화하는 논리 공백 | **반영함 (결정 ①).** 상수는 **유지** — 임계는 저장소 실측치가 아니라 상시 로드 총량 예산이고, 측정 집합만 정확해지고 예산은 그대로인 것이 맞다. 근거 문장만 두 표면에서 다시 썼다 |
| 2 | P3 | `## 범위 밖` 목록 미완 — `CLAUDE.local.md`·조상 디렉터리·`.claude/rules/*.md` | **부분 확인.** `CLAUDE.local.md` 제외 사유("gitignore라 팀 공유 아님")가 이 변경의 전제("출처를 구분하지 않으므로 합산")와 결이 다른 건 맞다 | **반영함 (결정 ②).** 범위 밖은 유지하되 사유를 **범위 판단**으로 교체하고 모순되는 근거는 철회. 조상 디렉터리·`.claude/rules`·전역 rules도 같은 방식으로 정리 |
...
| 6 | P3 | 라벨 포맷 — `…/CLAUDE.md(전역)` 구분자 없음 | **확인(미관).** | **수정함** — 경로와 `(전역)` 사이에 공백. 경로를 단언하는 테스트 3곳은 `includes(join(home,'CLAUDE.md'))`라 영향 없음을 확인 |
| 7 | P3 | TCC(`-context.md`)가 템플릿 원본 그대로 | **확인.** `context check`는 통과하나 재개 가치가 없다 | **남기기로 판단** — 아래 "미조치 판단" 참조 |

<!-- harness:review kind=claude scope=diff tip=51248955be98a59654eb074d2c8ff25c1215c32f at=2026-08-31T09:51:19Z -->

### 미조치 P3에 대한 판단 (2026-09-01)

남기는 것도 결정이므로 근거를 남긴다.

**발견 5 — 문서 드리프트: 남긴다(릴리스 시점 작업).** 실물을 확인했다.
`docs/harness-task-guide.html:290`은 `"0.23.0에서는 …"`이라는 **과거 서술**이고, 같은 파일
footer가 `0.23.0 기준 갱신`으로 고정돼 있다. 이 변경은 아직 **미출시**(`## [Unreleased]`)이므로
그 문장은 **지금 정확하다.** 지금 고치면 0.23.0 기준이라고 밝힌 문서가 미출시 동작을 설명하게
되어 더 나쁜 불일치가 된다. `what-changes-latest-version.html`·`harness-workflow-simulation.html`도
같은 구조다(각각 0.23.0으로 고정).

릴리스 때 이 작업을 할 사람을 위해 두 가지를 확인해 둔다 — 둘 다 리뷰가 짚지 않은 것이다:
- **`docs/harness-overview.html:218`은 생성물이다.** `scripts/generate-harness-overview.mjs`가
  `docs/harness-overview.template.html:217`에서 만든다. 생성물을 고치면 재생성 때 되살아나므로
  **템플릿을 고치고 재생성**해야 한다.
- **`-0.23.0.html` 스냅샷 3종**(`harness-overview-0.23.0.html`·`what-changes-0.23.0.html`·
  `harness-workflow-simulation-0.23.0.html`)은 **동결된 과거 기록이라 건드리지 않는다.**
  grep이 같은 문장을 거기서도 잡지만 고칠 대상이 아니다.

**발견 7 — 빈 TCC: 남긴다.** TCC는 SSOT가 아니라 **재개용 working-set 캐시**다. 이 task는
지금 종결되므로 재개할 세션이 없고, 종결 직전에 채우는 카드는 아무도 읽지 않는다.
`harness-team context check`도 현재 상태로 통과한다. 영속 가치가 있는 내용은 이미
spec·plan과 이 artifact에 있다.
