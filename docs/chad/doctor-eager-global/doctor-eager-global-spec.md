...
- **`CLAUDE.local.md`** — "Local" 스코프로 로드된다. **범위 판단으로 후속에 분리한다** —
  이번 변경은 "매 세션 무조건 로드되는 것을 빠짐없이 센다"를 한 번에 끝내는 것이 아니라,
  1차 출처로 로드가 확인된 소스부터 정확히 재는 것을 목표로 삼았다. 이 파일은 그 다음
  묶음이다. (이전에 "gitignore 대상이라 팀 공유 문서가 아니다"라고 적었던 사유는 **철회한다** —
  이 변경의 전제가 "context window는 바이트의 출처를 구분하지 않는다"이므로, 공유 여부는
  합산 여부의 근거가 될 수 없다. 전제와 모순되는 근거를 남겨 두면 다음 사람이 그 모순을
  다시 발견해야 한다.)
- **조상 디렉터리의 `CLAUDE.md` / `.claude/CLAUDE.md`** — 바이너리에서 조상 디렉터리를 훑는
  호출 지점을 확인했다(결정 6 참조). 다만 몇 단계까지 어떤 조건으로 올라가는지는 확인하지
  않았고, 측정 결과가 **점검 대상 디렉터리의 위치에 따라 달라진다**. 같은 수준의 1차 출처
  증거를 확보하기 전까지 범위 밖이다 — 결정 6이 세운 증거 기준을 이 항목에도 그대로 적용한다.
- **`.claude/rules/*.md` 중 `paths:` 스코프가 없는 것** — 바이너리가 같은 루프에서
  `type:"Project"`로 로드하고, 이 레포의 미러(`src/harness.mjs:471-472`)가 이를 `alwaysApply: true`로
  번역한다. 즉 항상 eager인 것이 맞다. 다만 **조건부 rule과 무조건 rule을 가르는 판정**이
  새로 필요해 측정 로직이 파일 읽기에서 frontmatter 파싱으로 넘어간다 — 범위가 다른 작업이라
  분리한다. (동봉 템플릿 4개는 전부 `paths:` 스코프가 있어 현재 실측 피해는 없다 — 확인함.)
- **전역 rules 디렉터리** (`~/.claude/rules`, User 스코프) — 위 항목과 같은 판정이 필요하고,
  프로젝트 밖이라 확인 비용도 더 든다. 같은 이유로 분리한다.
