...
**EOL 런타임 탈출은 구현 완료 · flake 해소 검증만 대기 중이다.** (2026-08-25 기준)
...
| **flake 해소 검증** | ⏳ **대기 — Node 24.20.0(2026-08-26) 릴리스 후** |
...
**다음 세션이 할 일은 하나뿐이다:** Node 24.20.0이 릴리스되면 패치된 런타임에서 **반복** 통과를
확인하고 그때 flake 해소를 선언한다. 현재 CI annotation은 `runtime v24.19.0`(**미패치**)을
기록하고 있으므로, 오늘의 green은 flake 해소의 증거가 아니다.
...
실제보다 나쁜 쪽으로 오도했다. 계측을 CI에 심어 둔 덕에 추정 대신 측정으로 닫았다. `engines`를 `>=22`로 올리는 것은 **하위 호환을 깨는** 변경이라
