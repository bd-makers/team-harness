...
인도할 것:
1. 브랜치 이름이 기본 브랜치가 아니어도 **HEAD 커밋이 `origin/<기본브랜치>`와 정확히 같으면**
   `--write`를 허용한다.
...
`git rev-parse origin/<candidate>`가 같으면, 이 브랜치에는 **로컬 커밋이 하나도 없다** —
원장을 써서 만드는 커밋은 `origin/main` 바로 위에 얹힌다. 기본 브랜치에서 쓰는 것과 결과가
같으므로 가드가 막으려는 충돌이 성립하지 않는다.
...
- [x] **Goal 명확도** (40%) — 목표가 한 문장으로 구체화되었는가?
      → "브랜치 이름 대신 HEAD 커밋이 `origin/<기본브랜치>`와 같은지를 보고 `--write`를 허용한다."
