...
### 2026-09-05 · 사용자 결정 (shipcheck 종결 · 잔여 리스크 처리)

ship 8단계에서 올린 결정 요청 2건에 대한 답이다. **게이트는 "통과"가 아니라 "결정"으로 닫혔다** —
리뷰어가 이 구분을 오해하지 않도록 여기 명시한다.

| # | 결정 요청 | 답 | 결과 |
|---|---|---|---|
| ① | `git -C <dir> push` 미매칭을 한계로 둘 것인가, 규칙을 추가해 덮을 것인가 | **한계 기록 유지** | 코드 변경 없음. 한계는 spec `## 설계 / 접근` · CHANGELOG `알려진 한계` · `tests/settings-permissions.test.mjs`의 `ASK_ENTRIES` 주석 세 곳에 남아 있다. 규칙 추가(subcommand 앞 와일드카드)는 목록이 배로 늘고 비가역 항목이 하나 더 생겨 기각 |
| ② | shipcheck #5를 돌려 게이트를 종결할 것인가 | **돌리지 않고 종결** | #4의 조치는 자기 이력을 세는 서술 한 줄 제거였고, 검증자가 독립 재산출한 수치(제품 표면 `6 files, +129` · sha pin 3건 · 신규 테스트 `6+2` · RED 재현)는 전부 일치했다. 남은 미검증분은 문서 산문 정밀도뿐이다 |

따라서 ship "준비 완료"는 **S1~S4가 4라운드 내내 pass**했고 S5의 마지막 조치가 서술 한 줄
제거라는 근거 위에서 선언한다. S5 최종 상태를 독립 재검증한 라운드는 없다.
