...
- `buildErrorPacket`이 5키를 **강제**한다: `cause`·`retry`·`safeDefault`·`stop`이 비어 있지 않은
  string이 아니면 throw, `alternatives`는 string 배열이어야 한다. 잘못된 생산자는 개발 시점에
  터진다.
...
- **escalation packet**: 자동 실행을 멈추고 사람(또는 상위 에이전트)에게 결정을 넘길 때 함께
  ...
  사람용(`CLAUDE.md` §5-A 복잡도 게이트). 이 task는 둘 다 PDF 5항목에 맞춘다.
