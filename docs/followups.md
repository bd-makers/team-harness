...
**남은 것: 2 · 4 · 6 · 7.** 2번은 선행 검증과 수리가 끝나 **결정(B/C) 하나만** 남았다. 나머지는 급하지 않다.
...
> 2026-09-12: 선행 검증(A)과 수리(A')를 **모두 끝냈다.** Codex 훅은 이제 실제로 주입한다(0.38.3).
> **남은 것은 B(확장) 냐 C(비대칭 유지) 냐, 결정 하나뿐이다.**
...
### 선택지 — A' **완료** (2026-09-12, task `codex-hook-injection-fix`, 0.38.3)

| | 무엇 | 상태 |
|---|---|---|
| ~~A. 검증 먼저~~ | project-level 훅의 로딩 조건 규명 | **완료** (`codex-project-hooks-probe`) |
| ~~A'. 고쳐서 살린다~~ | 주입 형식을 JSON `additionalContext` 로 · 신뢰 부재를 doctor 경고로 · README 정정 | **완료** — `session-context --codex-hook`, `Codex hook trust` 경고, `migrate` 가 옛 커맨드를 제자리 업그레이드. **실제 codex 세션이 `[harness]` 줄을 인용하는 것으로 검증** |
| **B. 확장** | `PreToolUse`(보호 경로·위험 git)와 관측을 Codex 에도 배선 | **미결정** — 이제 비용은 "이벤트를 더 거는 것"뿐이다. 훅 스크립트·wire protocol 이 Claude 와 같아 재사용 가능 |
| **C. 비대칭 유지** | 현 상태(SessionStart 1종)에서 멈춘다 | **미결정** — 고르면 이 항목을 종결한다 |

**남은 결정은 B 냐 C 냐 하나뿐이다.** 판단 재료:
- **B 의 실제 비용**: 훅 스크립트 3종(`protect-files.sh`·`block-dangerous-git.sh`·`observe-tools.mjs`)은
  Codex 의 wire protocol 이 Claude 와 같아 거의 그대로 쓸 수 있다. 늘어나는 것은 템플릿·doctor·테스트 표면이다.
- **B 의 역할 충돌**: D2 는 Codex 를 **리뷰어(read-only)** 로 규정한다. 쓰기 차단 훅을 다는 것이
  그 역할과 맞는지가 진짜 질문이다 — 기술 문제가 아니라 팀 계약 문제다.
- **C 의 비용**: 0. 지금 상태가 정직하게 문서화돼 있다(README 인용 블록).
