...
6. **다이어그램 (옵트인)** — `--no-diagram`이 없으면 `AskUserQuestion`으로 **한 번만** 묻는다:
   ...
   - **Probe** — Claude Code에서 `diagram-design` 스킬(`/diagram-design:diagram-design`)이
     노출되는지 확인한다. 이 스킬은 이 플러그인의 것이 아니라 **별도 마켓플레이스의 Claude Code
     전용 플러그인이며 머신별 설치**라 없을 수 있다. 없으면 설치 명령을 단정해 안내하지 말고
     (존재하지 않는 설치 경로를 안내하는 결함 방지) 해당 마켓플레이스에서 설치·활성화되어 있는지
     확인하도록 안내한다.
