...
1. **설비 감지는 CLI가 한다** — 러너·프레임워크·TypeScript·Testing Library 계열·목킹·커버리지를
   ...
   - CLI는 read-only다. 설치하지도, 러너를 추천하지도 않는다 — 그건 아래 3번이 할 일이다.
2. **기존 테스트 2~3개 샘플링** — 이미 있는 `*.test.*` / `*.spec.*` / `__tests__/`
   ...
   - **팀 컨벤션이 이 계약과 충돌하면 팀 컨벤션을 따른다** (스냅샷 정책 등은 §4 예외 참조).
...
4. **스택 요약을 5줄 이내로 보고**한 다음 테스트 작성에 들어간다. CLI의 `summary`를 그대로 쓰고,
   CLI가 알 수 없는 것(2번에서 추출한 팀 컨벤션)만 덧붙인다.
   예: `러너: Vitest · 프레임워크: React(web) · TS: 예 · msw: 있음 · 컨벤션: co-location, it()/한국어`.
