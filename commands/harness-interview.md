...
2. **선행 채점** — 질문을 만들기 전에, spec 텍스트 증거만으로 채점표를 만든다. 차원은
   ...
   - **Goal의 pass 근거는 두 문장이다** — 기대 결과(무엇이 달성되는가)와 문제(오늘 무엇이 안 되는가 ·
     누가 영향받는가). 문제 문장을 인용할 수 없으면 Goal은 pass가 아니라 na다 — 결과만 있는 목표는
     해결책이 문제를 찾는 상태라 "너무 넓지 않은가"를 판정할 기준이 없다.
3. **fail/na 차원만** 대상으로 각 2~3개씩 질문을 만든다. 질문 각도:
   - **Goal**: "오늘 사용자가 무엇을 못 하는가? 누가 영향받는가?", "달성됐는지 어떻게 알 수 있는가?",
     "지금 정의는 너무 넓지 않은가?"
