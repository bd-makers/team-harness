...
5. 소스별 수집:
   ...
   - **Interview**: task 이름을 시드로 생성형 인터뷰를 진행한다 — Goal / Constraint / Success / Context
...
7. Ambiguity 자가진단을 **초안 자기 평가**로 갱신한다 — 근거가 있는 항목만 `- [x]`로 체크하고 근거를 한 줄
   ...
   검증을 거쳐 이뤄진다. 미체크 또는 `(unresolved)` 항목이 있으면 `/harness-interview` 인계를 안내하고,
   없으면 plan 작성 단계로 인계하되 검증이 필요하면 `/harness-interview`를 권한다.
...
## 종료 조건
- spec.md 초안 저장 + 자가진단 자기 평가 완료. 미체크/unresolved 항목이 있으면 `/harness-interview`
  인계 안내까지. 유효 소스가 없으면 초안 없이 종료를 보고한다.
